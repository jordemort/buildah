...
SPCCMD="${SPCCMD:-./hack/spc_build_ubuntu.sh}"
DISTRO="${DISTRO:-ubuntu}"
FQIN="docker.io/lsm5/travis_${DISTRO}:xenial"
...
TRAVIS_BUILD_DIR="/home/lsm5-bot/$REPO_NAME"
...
ENV_ARGS="-e HOME=/home/lsm5-bot -e SHELL=/bin/bash -e SPC=true -e DISTRO=$DISTRO -e WORKDIR=$WORKDIR -e DECRYPTION_PASSPHRASE=$DECRYPTION_PASSPHRASE -e GPG_KEY_PASSPHRASE=$GPG_KEY_PASSPHRASE"
...
echo
set -x
sudo docker run --user lsm5-bot -t $SPC_ARGS $VOL_ARGS $ENV_ARGS $TRAVIS_ENV $FQIN $SPCCMD
